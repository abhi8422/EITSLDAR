...
/*                FragmentManager fragmentManager = getSupportFragmentManager();
                ...
                if (menuId == R.id.inspect_routes) {
                    Timber.e("(Delete Route Drawer)click on inpsected route");
                    fraglayout.setCurrentItem(0);
                    finish();
                    drawerLayout.closeDrawer(GravityCompat.START);
                } else if (menuId == R.id.down_routes) {
                    Timber.e("(Delete Route Drawer)click on download route");
                    fraglayout.setCurrentItem(1);
                    finish();
                    drawerLayout.closeDrawer(GravityCompat.START);
                } else if (menuId == R.id.upload_routes) {
                    Timber.e("(Delete Route Drawer)click on upload route");
                    fraglayout.setCurrentItem(0);
                    finish();
                    drawerLayout.closeDrawer(GravityCompat.START);
                } else if (menuId == R.id.bt_config) {
                    Timber.e("(Delete Route Drawer)click on TVA Connect");
...
//                    Toast.makeText(getApplicationContext(),"Redirecting to bluetooth configuration",Toast.LENGTH_SHORT).show();
                    ...
                    Timber.e("(Delete Route Drawer)click on delete route");
                    drawerLayout.closeDrawer(GravityCompat.START);
                } else {
                    finish();
                }
