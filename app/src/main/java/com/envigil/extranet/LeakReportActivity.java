...
public class LeakReportActivity extends AppCompatActivity implements View.OnClickListener {
   ...
   public  static int RouteId,InvId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(leakedit_Flag){
            Timber.e("Flow Coming from leak edit");
            ...
            public void onClick(View view) {
                ...
                if(!(LeakPathSpinner.getSelectedItem()=="Select Leak Path ")){
                    ...
                    if (count == 0){
                        ...
                        Timber.e("(count =0) Inserting new  leak values are LeakID="+LeakId +",InvID="+InvId+",InvTag="+InvTag+",LeakPathId="+LeakPathTypeid+",LeakTypeId="+LeakTypeId+",CompId="+leakcompId+",leakBit1="+leakBit1+",leakBit5="+leakBit5+",LeakFloat1="+LeakFloat1+",LeakRate="+ leakRate+",LeakTime="+LeakTime +",DateTime="+DateTime+",LeakLat="+leakLAT+",LeakLng="+leakLNG+",false"+",ImagePath="+Img_Path);
                    }
                    else {
                        sqLiteHelper.UpdateLeaks(InvId,LeakPathTypeid,LeakTypeId,leakBit1,leakBit5,leakRate,LeakTime,DateTime,Img_Path);
                        Timber.e("(count!=0)Update Leak InvID="+InvId+",LeakPathTypeid="+LeakPathTypeid+",LeakTypeId="+LeakTypeId+",leakBit1="+leakBit1+",leakBit5="+leakBit5+",leakRate="+leakRate+",LeakTime="+LeakTime+",DateTime="+DateTime+",Img_Path="+Img_Path);
                    }
...
//                startActivity(new Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS));
    ...
    public void OpenRepair(View view) {
        Timber.e("Navigate To leak repair Request");
...
//        leakReportBottom.dismiss();
    ...
    public void Cancel(View view) {
        Timber.e("Click on cancel for repair request");
        if(leakedit_Flag){
            leakedit_Flag=false;
            int routeid=RouteId;
            Timber.e("go to ShowLeaksActivity");
...
//            grid=ComponentReading.grid;
                if (grid){
                    Timber.e("go to ComponentsTable");
                    startActivity(new Intent(LeakReportActivity.this, ComponentsTable.class).putExtra("SubId",SubID).putExtra("RouteID",RouteId).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK));
                    finish();
                }
                else {
                    Timber.e("go to ComponentDashboard");
                    startActivity(new Intent(LeakReportActivity.this, ComponentDashboard.class).putExtra("SubId",SubID).putExtra("RouteID",RouteId));
                    finish();
                }            finish();
            }
                ...
                boolean next =true;
                Timber.e("go to ComponentReading CompID="+leakcompId+",SubId="+SubID+
                        ",RouteID="+RouteId+",InvID="+InvId+",Grid="+grid+",next="+next);
                startActivity(new Intent(LeakReportActivity.this, ComponentReading.class)
                        .putExtra("CompId",leakcompId)
                        .putExtra("SubId",SubID).putExtra("RouteID",RouteId)
                        .putExtra("InvID",InvId).putExtra("next",next)
                        .putExtra("Grid",grid));
                finish();
            }
        }
