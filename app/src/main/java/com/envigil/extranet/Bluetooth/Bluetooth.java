...
public class Bluetooth extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //logging file
        logger = Logger.getLogger("BleLog");

        String logDirectory,dayFolder;
        File logcatfile,tvaData;
        FileHandler fh;
        Process process;
        try {
            // This block configure the logger with handler and formatter
            logDirectory = Environment.getExternalStorageDirectory()+File.separator + "ExtranetDAEP_Log";
            SimpleDateFormat format=new SimpleDateFormat("EEE:d-MMM-yy");
            dayFolder=logDirectory+File.separator +format.format(Calendar.getInstance().getTime());
            logcatfile = new File(dayFolder, "BleLog"+System.currentTimeMillis()+".txt");
            tvaData=new File(dayFolder,"TVAData"+System.currentTimeMillis()+".txt");
            fh = new FileHandler(tvaData.getPath());
            logger.addHandler(fh);
            SimpleFormatter formatter = new SimpleFormatter();
            fh.setFormatter(formatter);
            //clearing the log buffer
             process= Runtime.getRuntime().exec("logcat -c");
             //filtering the log by package name
             process = Runtime.getRuntime().exec(" logcat | grep <com.envigil.extranet.Bluetooth> -f " + logcatfile);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                try {
                    ...
                    clientClass.start();
                    logger.info("Connecting To Devices :: "+btArray[position].getName());
        ...
        navigationView.setNavigationItemSelectedListener(new NavigationView.OnNavigationItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                ...
                if (menuId == R.id.home_app){
                    Timber.e("(Ble Drawer) click on home app");
                    ...
                    Timber.e("(ble drawer)click on inspect route");
                    finish();
                    drawerLayout.closeDrawer(GravityCompat.START);
                }
                else if (menuId == R.id.down_routes){
                    Timber.e("(ble drawer)click on download route");
                    fraglayout.setCurrentItem(1);
                    finish();
                    drawerLayout.closeDrawer(GravityCompat.START);
                }
                else if (menuId == R.id.upload_routes){
                    Timber.e("(ble drawer)click on Upload route");
                    finish();
                    drawerLayout.closeDrawer(GravityCompat.START);
                }
                else if (menuId == R.id.bt_config){
                    Timber.e("(ble drawer)click on TVA Connect");
